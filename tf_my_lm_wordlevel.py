...
# configs
keep_prop = .9  # of course for training .. give 1 at validation
sequence_length = 48
batch_size = 200
vocabulary_size = 15000
embedding_size = 128  # word level language model
...
# data_root_path = 'D:\preferences\jetBrains\pycharm\\udacity\\text8.data'
data_root_path = './text8'
# http://mattmahoney.net/dc/text8.zip

concat_text = read_data_files_as_words(data_root_path)
data_length = len(concat_text)
print("data length in chars ", data_length)
print("-" * 100 + '\n', ''.join(concat_text[:20]))

encoded_data, counts, dictionary, reverse_dictionary = build_word_dataset(concat_text, vocabulary_size)
...
with graph.as_default():
    keep_prop_tf = tf.placeholder(tf.float32, name='keep_prop_tf')
    embeddings = tf.Variable(tf.random_uniform([vocabulary_size, embedding_size], -1.0, 1.0), name='embeddings')

    softmax_weights = tf.Variable(tf.truncated_normal([internal_state_size, vocabulary_size], stddev=1.0 / math.sqrt(embedding_size)), name='weights')
    softmax_biases = tf.Variable(tf.zeros([vocabulary_size]), name='biases')

    # repeat it stacked_layers
    cells = [tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.GRUCell(internal_state_size), output_keep_prob=keep_prop_tf) for _ in range(stacked_layers)]
    multi_cell = rnn.MultiRNNCell(cells, state_is_tuple=True)
    ...
    with tf.variable_scope('train'):
        ...
        batch_seq_labels = tf.placeholder(tf.int32, [batch_size, sequence_length])  # [ batch_size, sequence_length ]
        ...
        logits_train = tf.nn.xw_plus_b(out_states_flattened_train, softmax_weights, softmax_biases)  # [ batch_size x sequence_length, embedding_size >>> project on vocabulary space ]
        ...
        # labels
        labels_flattened = tf.reshape(batch_seq_labels, [-1])  # [ batch_size x sequence_length ]
        ...
        gradients, _ = tf.clip_by_global_norm(gradients, clip_norm=grad_clip)  # 1 to 5
        ...
        accuracy = tf.reduce_mean(tf.cast(tf.equal(batch_seq_labels, tf.cast(batch_seq_pred_train, tf.int32)), tf.float32))
    ...
    for batch, label, epoch in minibatch_generator(train, nb_epochs=num_epochs, gen_batch_size=batch_size, gen_seq_len=sequence_length):
        ...
        if (current_time - summary_last) > validation_every_mins * minute:
            ...
            for k in range(2000):
                ...
                sample = sample_from_probabilities(probabilities, topn=1000 if epoch <= 1 else 5000, vocabulary_size=vocabulary_size, is_word_level=True)
        ...
        if (current_time - checkpoint_last) > chechpoint_every_mins * minute:
            ...
            make_zip_results("word", step, outputFileName)
